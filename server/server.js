...
app.post("/saveUserData", async (req, res) => {
  try {
    ...
    const { name, age, email, phone, selectedBatch } = req.body;
    ...
    if (existingUser) {
      ...
      if (daysSinceLastUpdate < 30) {
        return res
          .status(201)
          .json({ message: "You Can Update Data only once a month" });
      }

      // If user exists and the update is allowed, update the existing user
      existingUser.name = name;
      existingUser.age = age;
      existingUser.phone = phone; // Add phone field to existing user
      ...
      const newUser = new userschema({
        ...
        age,
        email,
        phone, // Add phone field to new user
        selectedBatch,
      });
      await newUser.save();
    }
